  ...
  <div class="stock-query">
    <el-card class="query-card card-hover card-glow">
      ...
      <el-table :data="filteredStock" border style="width: 100%" v-loading="loading">
        <el-table-column prop="in_time" label="入库时间" width="120" sortable>
          <template #default="{row}">
            {{ formatDate(row.in_time) }}
          </template>
        </el-table-column>
        <el-table-column prop="out_time" label="出库时间" width="120" sortable>
          <template #default="{row}">
            {{ row.out_time ? formatDate(row.out_time) : '-' }}
          </template>
        </el-table-column>
...
const exportToExcel = async () => {
  try {
    ...
    const exportData = exportStockData.map(item => {
      ...
      // 格式化出库时间
      const formattedOutDate = item.out_time ? formatDate(item.out_time) : '-';
      
      // 返回格式化后的数据行
      return {
        '入库时间': formattedDate,
        '出库时间': formattedOutDate,
        '食材名称': item.name,
        '分类': categoryName,
        '供应商': supplierName,
