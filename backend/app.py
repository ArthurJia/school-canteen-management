...
def create_supplier():
    try:
        ...
        cursor.execute('''
        INSERT INTO suppliers (name, contact, phone, shortName, supply_items)
        VALUES (?, ?, ?, ?, ?)
        ''', (
            data['name'],
            data['contact'],
...
def update_supplier(supplier_id):
    try:
        ...
        cursor.execute('''
        UPDATE suppliers
        SET name = ?, contact = ?, phone = ?, shortName = ?, supply_items = ?
        WHERE id = ?
        ''', (
            data['name'],
            data['contact'],
...
def get_all_monthly_suppliers():
    try:
        ...
        cursor.execute('''
        SELECT s.id, s.name, s.contact, s.phone, s.shortName, s.supply_items, s.created_at, ms.year, ms.month
        FROM suppliers s
